...
namespace NetCoreAngularCRUDApp.Data
{
    public class BlogPostRepository : IBlogPostRepository
    {
        ...
        {
            return context.BlogPost.OrderByDescending(p => p.PostId).AsEnumerable();  
        }

        public BlogPost Get(int id)
        {
            return context.BlogPost.FirstOrDefault(p => p.PostId == id);
        }

        public async Task<BlogPost> GetAsync(int id)
        {
            return await context.BlogPost.FindAsync(id);
        }
